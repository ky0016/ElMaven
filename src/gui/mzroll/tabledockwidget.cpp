...
QByteArray TableDockWidget::run_qt_process(QString command){
...
QByteArray TableDockWidget::run_system_process(QString command){
...
QString TableDockWidget::get_run_id(QByteArray result){
...
QString TableDockWidget::get_patch_id(QByteArray result){
    QList<QByteArray> test_list = result.split('\n');
    int size = test_list.size();
    QByteArray result2 = test_list[size-2];
...
QStringList TableDockWidget::get_system_urls(QString filename){
...
QStringList TableDockWidget::get_project_upload_url_commands(QByteArray result2,QStringList filenames){
    QStringList upload_commands ;
    ...
    for (int i=0; i < filenames.size(); ++i){
        ...
        QString random_filename = "random_file_name.json";
...
QString TableDockWidget::get_urls(QByteArray result){    
    QList<QByteArray> test_list = result.split('\n');
    int size = test_list.size();
    QByteArray result2 = test_list[size-2];
    QJsonDocument doc(QJsonDocument::fromJson(result2));
...
int TableDockWidget::authenticate_login(){
    int status;
...
void TableDockWidget::exportPolly() {
    LOGD;
    int status = authenticate_login();
    if (status==0){
        qDebug()<<"user needs to log in..";    
        // QWebView *view = new QWebView();
        // view->load(QUrl("http://testpolly.elucidata.io/auth/login"));
        // view->show();
        // QWebFrame *frame = view->page()->currentFrame();
        // ->toHtml()
        // qDebug()<<"page html  "<<frame;
        // qDebug()<<"page html elements  "<<frame.findAllElements();
        QString command = QString("mithoo authenticate %1 %2").arg("rohit.negi@elucidata.io").arg("Moderator.1");
        QByteArray result = run_qt_process(command);
        int status_inside = authenticate_login();
        if (status_inside==1){
    ...
    */
    ...
    if (status ==1){
        ...
        QString command2 = QString("mithoo createProject %1").arg("test_project1");
        ...
        qDebug() << "polly user credentials are not valid....exiting \n\n";
        return ;
    }
}
